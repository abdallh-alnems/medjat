...
| `medjat-alerts.rules.yml` | `/etc/prometheus/rules/medjat-alerts.yml` | the 12 rules |
| `medjat-alerting-selftest.sh` | `/usr/local/bin/`, cron `*/15` | pings healthchecks only if the pipeline can deliver |
| `alertmanager.yml` | `/etc/prometheus/alertmanager.yml` | grouping, repeat, routing |
| `medjat-alert-sender.py` | `/usr/local/bin/` | webhook → WhatsApp + Telegram |
| `medjat-alerts.service` | `/etc/systemd/system/` | runs the sender on :9099 |
| `medjat-node-metrics.sh` | `/usr/local/bin/`, cron `*/5` | backup age + nginx 5xx |
| credentials | `/etc/medjat-alerts.env` (0640, **not in git**) | channel keys |
...
- **critical** (WhatsApp + Telegram): API down (through Cloudflare *and* at the
  ...
  broken"), web app down, `mysql`/`nginx`/`php-fpm`/`medjat-web` not running,
  disk under 7%.
...
- **heartbeat**: a `Watchdog` rule that always fires. It is the answer to the
  ...
  Cairo), remembered in `/var/lib/medjat-alerts/heartbeat.date`. A bare
...
`medjat-alert-sender.py` receives the Alertmanager webhook, formats one Arabic
...
**Telegram** (`@medjat_alerts_bot`, live) — official API, free, no extra phone
...
```bash
sudo -u medjat-alerts /usr/local/bin/medjat-alert-sender.py --test
```
...
Project `medjat`, free Hobbyist plan (20 checks, we use 7):

| Check | Expected | Grace | Pinged by |
|---|---|---|---|
| `medjat-server-alive` | every 15 min | 20 min | cron in `/etc/cron.d/medjat-monitoring` |
| `medjat-backup` | `0 2 * * *` | 3 h | `medjat-backup.sh` |
| `medjat-absences` | `50 23 * * *` | 2 h | `medjat-cron-run.sh` |
| `medjat-leave-rollover` | `0,30 0 * * *` | 2 h | `&&` in `/etc/cron.d/medjat` |
| `medjat-daily-alerts` | `0 7 * * *` | 2 h | `medjat-cron-run.sh` |
| `medjat-kiosk-purge` | `30 3 * * *` | 3 h | `medjat-cron-run.sh` |
| `medjat-alerting-alive` | every 15 min | 20 min | `medjat-alerting-selftest.sh` |

`medjat-alerting-alive` is the one that watches the watcher. It pings only
...
stopping `medjat-alerts.service` and confirming the check reported `/fail`.
...
1. The three cron wrappers ran `curl -s` with no `-f`, so they exited 0 whatever
   ...
   "the job ran fine" while the job failed. `medjat-cron-run.sh` now requires a
   2xx **and** a body saying the endpoint finished (`"status":"success"` or
   `"ok"`), and pings `/fail` with the response otherwise.
2. `medjat-backup.sh` piped `mysqldump | gzip` without `pipefail`. A mysqldump
...
`medjat-cron-run.sh` and its wrappers are `750 root:www-data`. Setting them
...
the server went quiet, one from Medjat Alerts means the server is alive and
complaining.

Check names stay ASCII (`medjat-backup`) on purpose: they are identifiers that
...
project API key is in [[medjat-server-credentials]].
...
`api.medjatapp.com` (set to treat **401** as success) closes that last gap.
...
- **`/var/log/medjat-cron.log` had no rotation** and grew forever. Weekly, 8
  kept, in `/etc/logrotate.d/medjat`. Needs `su root syslog` — without it
