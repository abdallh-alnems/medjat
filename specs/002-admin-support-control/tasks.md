...
- Admin app (Flutter): `front_end/medjat_admin/lib/`
- Other apps: `front_end/medjat_app/` (Employee), `front_end/medjat_central/` (HR — no change)
...
- [x] T003 [P] Run `flutter pub get` in `front_end/medjat_admin` and confirm the app builds on the current branch
...
- [x] T004 Add support + app-control endpoint URLs to `front_end/medjat_admin/lib/core/constant/id/app_links.dart` (support: list/messages/reply/status; app control: get/set; device register) and mark the deprecated `forceUpdateTrigger` getter for removal
- [x] T005 Add new route names to `front_end/medjat_admin/lib/core/constant/routes/app_routes.dart` (`supportInbox`, `supportThread`, `appControl`) and remove the obsolete `forceUpdate` route
...
- [x] T007 [P] [US1] Create `front_end/medjat_admin/lib/data/model/support_ticket_model.dart` (id, tenantId, tenantName, subject, category, priority, status, lastMessageAt, lastMessagePreview, unreadForSupport) per data-model.md
- [x] T008 [P] [US1] Create `front_end/medjat_admin/lib/data/model/support_message_model.dart` (id, ticketId, senderType, body, createdAt)
- [x] T009 [US1] Create `front_end/medjat_admin/lib/data/data_source/remote/support_data/support_data.dart` with `list({page,status,tenantId})`, `messages(ticketId,{afterId})`, `reply(ticketId,body)`, `setStatus(ticketId,status)` using `Get.find<CRUD>()`

### Flutter — controller

- [x] T010 [US1] Create `front_end/medjat_admin/lib/logic/controller/support/support_controller.dart`: inbox load + status/tenant filters, thread load (mark-read on open), send reply, change status, and a ~5s polling timer using `after_id` while a thread is open (stop on close/background) — meets SC-003

### Flutter — UI

- [x] T011 [P] [US1] Create `front_end/medjat_admin/lib/view/screen/support/support_inbox_screen.dart` (ticket list ordered by last activity, unread badge from `unreadForSupport`, status + company filters, empty/loading states)
- [x] T012 [P] [US1] Create `front_end/medjat_admin/lib/view/screen/support/support_thread_screen.dart` (chat bubbles user vs support, reply input ≤5000 chars text-only, status action menu, deactivated-tenant notice per FR-011)

### Flutter — wiring

- [x] T013 [US1] Register `SupportBinding` (SupportData + SupportController) and `GetPage`s for `supportInbox`/`supportThread` in `front_end/medjat_admin/lib/core/constant/routes/app_pages.dart`
- [x] T014 [US1] Add a "Support / الدعم" entry to the dashboard navigation in `front_end/medjat_admin/lib/view/screen/dashboard/dashboard_screen.dart` linking to `supportInbox`
...
- [x] T018 [P] [US2] Ensure the Employee app reads `medjat_app_min_version` + `medjat_app_maintenance_enabled` — mirror the HR app's `UpdateService`/`MaintenanceGate` in `front_end/medjat_app/lib/core/` (add gate/service if missing); fall back to a safe non-locking state on RC failure (FR-019/SC-008)
- [x] T019 [P] [US2] Add a `medjat_admin_min_version` force-update check to `front_end/medjat_admin` (version-only, no maintenance gate)

### Flutter (admin) — app control screen

- [x] T020 [P] [US2] Create `front_end/medjat_admin/lib/data/model/app_control_model.dart` (key, name, minVersion, maintenance nullable, supportsMaintenance)
- [x] T021 [US2] Create `front_end/medjat_admin/lib/data/data_source/remote/app_control_data/app_control_data.dart` with `get()` and `set({app, minVersion?, maintenance?})`
- [x] T022 [US2] Create `front_end/medjat_admin/lib/logic/controller/app_control/app_control_controller.dart` (load apps, edit version, toggle maintenance for Employee/HR only, client-side version validation per FR-017)
- [x] T023 [US2] Create `front_end/medjat_admin/lib/view/screen/app_control/app_control_screen.dart` (per-app card: min-version field + save; maintenance toggle shown only when `supportsMaintenance`; current-state display)
- [x] T024 [US2] Register `AppControlBinding` + `GetPage` for `appControl` and remove the deprecated `ForceUpdateBinding`/route in `front_end/medjat_admin/lib/core/constant/routes/app_pages.dart`; delete `force_update_*` data/controller/screen
...
- [x] T026 [US3] Add a reusable confirmation dialog in `front_end/medjat_admin/lib/core/shared/dialogs/` (or extend existing) that names the affected app and the impact
...
- [x] T032 [USPush] Add `firebase_core` + `firebase_messaging` to `front_end/medjat_admin/pubspec.yaml`; add platform config (`google-services.json`, `GoogleService-Info.plist`); initialize Firebase in `front_end/medjat_admin/lib/main.dart`
- [x] T033 [USPush] Create `front_end/medjat_admin/lib/data/data_source/remote/device_data/device_data.dart` + register the FCM token on login/app-start (request notification permission), calling the device-register endpoint
...
- [x] T037 [P] Run `flutter analyze` on `front_end/medjat_admin` (and any touched app) and resolve lints; confirm RTL/Arabic strings render
