...
- Employee web: `frontend/web/manager/src/`
- Admin app: `frontend/mobile/manager/lib/`
...
- [X] T002 [P] Create the isolated employee route group at `frontend/web/manager/src/app/(employee)/layout.tsx` with its own providers, importing nothing from the `(app)` admin tree
- [X] T003 [P] Create the employee API client at `frontend/web/manager/src/lib/api/employee.ts` — separate axios instance sending `X-Employee-Token`, never the admin Firebase token
- [X] T004 [P] Create the feature folder `frontend/web/manager/src/features/employee-attendance/` with `schemas.ts` (Zod) and `hooks.ts` (TanStack Query) stubs
...
- [X] T024 [P] [US1] Implement the browser identity cookie in `frontend/web/manager/src/features/employee-attendance/device-id.ts` — random UUID, long-lived, sent as `device_id`
- [X] T025 [US1] Build the activation page `frontend/web/manager/src/app/(employee)/activate/page.tsx` — phone, activation code, choose and confirm a 6-digit PIN, with client-side Zod mirroring the server rules
- [X] T026 [US1] Build the sign-in page `frontend/web/manager/src/app/(employee)/login/page.tsx` — phone (remembered locally) + PIN, with a distinct locked-account state pointing the employee at their administrator
- [X] T027 [US1] Build the attendance page `frontend/web/manager/src/app/(employee)/attendance/page.tsx` — render state from `web_status.php`, request geolocation, warm the camera only when `photo_required`, and show one primary button
...
- [X] T032 [P] [US1] Vitest unit tests in `frontend/web/manager/src/features/employee-attendance/__tests__/` — PIN validation, attendance state machine, device-id persistence
- [ ] T033 [US1] Playwright e2e in `frontend/web/manager/e2e/employee-attendance.spec.ts` — activate → check in → check out → confirm the session is dead, with geolocation and camera mocked
...
- [X] T040 [US2] Build the settings UI in `frontend/mobile/manager/lib/view/screen/settings/` — the two toggles, per-category access, and the honest disclosure of what the browser cannot verify (WiFi BSSID, mock-location, face) with the branch warning from T036
...
- [X] T049 [P] Run `flutter analyze lib` in `frontend/mobile/manager` (bare `flutter analyze` reports phantom errors from FlutterFire examples under `build/`)
- [X] T050 [P] Run `npm run lint` and `npm test` in `frontend/web/manager`
