...
### الموظفون
- **الموظفون** (Employees): إضافة/تعديل/عرض تفاصيل الموظف، مستنداته، وتسوية مستحقاته (Settlement).
- **تسجيل القياسات الحيوية** (Biometric Enrollment): تسجيل **بصمة الوجه** (Face) و**بصمة الإصبع** (Fingerprint).
...
- **طرق الحضور** (Attendance Method): QR / الموقع الجغرافي / التعرّف على الوجه — مع نظام **تجاوزات (Overrides)** بترتيب: الموظف ← الفئة (اتحاد) ← الفرع ← الشركة (Tenant).
...
- **المحطات** (Stations): محطات التعرّف على الوجه (Kiosk)، إعداداتها وسجلّات التعرّف.
...
│   │                            settlement, schedule, station, biometric, support …)
...
- **الشبكة:** فئة `core/class/crud.dart`؛ كل عمليات الكتابة تستخدم **POST** (PUT غير موثوق على Hostinger).
...
REST API بلغة **PHP 8.x** في `backend_medjat/` — كل endpoint في ملف منفصل داخل `backend_medjat/app/`، والـ queries في `backend_medjat/core/queries/`. القاعدة **MySQL 8** (محليًا عبر MAMP على المنفذ `8889`، المستخدم/كلمة المرور `root/root`، قاعدة `medjat`).

> بعض الميزات (التعديلات الجماعية، تجاوزات طرق الحضور، ترحيل الإجازات المتقدّم، الجلسة الواحدة النشطة) تتطلّب **migrations مكتوبة يدويًا** على قاعدة MySQL 8 الحيّة، لأن `schema.sql` يستخدم صياغة `ADD COLUMN IF NOT EXISTS` الخاصة بـ MariaDB.
