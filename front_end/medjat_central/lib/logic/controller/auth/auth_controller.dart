  ...
  Future<void> login({
    ...
    if (response['status'] == StatusRequest.success) {
      ...
      if (statusCode == 401) {
        ...
        message = 'ليس لديك صلاحية الدخول للوحة الإدارة';
      }
  ...
  Future<void> loadProfile() async {
    ...
    if (response['status'] == StatusRequest.success && response['data'] != null) {
      ...
      await TokenStorageService.saveUserData(
          '{"id":${user!.id},"name":"${user!.name}","email":"${user!.email}","role_key":"${user!.roleKey}","permissions":${user!.permissions},"tenant_id":${user!.tenantId},"branch_id":${user!.branchId}}');
      update();
    }
  }
